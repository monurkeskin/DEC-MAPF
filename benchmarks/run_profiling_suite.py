"""Fixed-Instance Core Profiling Suite (O01).

Benchmarks corrected simulation core across:
1. Topologies: Sparse, Corridor Bottleneck, Dense Obstacles.
2. Granularity: metrics-only, events, full-trace.
Reports wall time, CPU time, peak memory, and verification correctness.
"""
...
def profile_case(
    ...
    for aid, s in starts.items():
        sim.add_agent(HeatMapAgent(aid, s, goals[aid], cfg.initial_tokens, cfg.fov_size))
    ...
    executed_paths = {aid: MAPFPath(points=pts) for aid, pts in res["path_history"].items()}
...
def run_profiling() -> list[dict[str, Any]]:
    ...
    results = []
    print(f"{'Case':<22} | {'Level':<12} | {'Wall (ms)':<10} | {'Peak (KiB)':<10} | {'Valid':<5} | {'Frames'}")
    print("-" * 75)

    for name, w, h, starts, goals, obs in cases:
        for level in [RecordingLevel.METRICS_ONLY, RecordingLevel.EVENTS, RecordingLevel.FULL_TRACE]:
            r = profile_case(name, w, h, starts, goals, obs, level)
            results.append(r)
            print(
                f"{r['case']:<22} | {r['recording_level']:<12} | {r['wall_time_ms']:<10} | "
                f"{r['peak_mem_kib']:<10} | {str(r['valid']):<5} | {r['frames_retained']}"
            )
...
if __name__ == "__main__":
    raise SystemExit("Archived unqualified runner. Use mapf batch plan/run/analyze; see docs/EXPERIMENTS.md. Historical outputs are preserved.")
    run_profiling()
