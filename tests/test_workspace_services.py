...
def test_supervisor_sigkill_stops_child_and_recovers_attempt(tmp_path):
    ...
    try:
        ...
        entered = tmp_path / "staging" / "solver-entered.pid"
        deadline = time.monotonic() + 15
        while not (ready.exists() and entered.exists()) and time.monotonic() < deadline:
            time.sleep(0.03)
        assert ready.exists() and entered.exists(), (
            "Owned solver never entered execution"
        )
        data = json.loads(ready.read_text())
        pid = data["worker_pid"]
        assert int(entered.read_text()) == pid
        os.kill(pid, 0)
        proc.kill()
        proc.join(timeout=2)
