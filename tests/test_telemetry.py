...
def test_telemetry_event_serialization() -> None:
    move = AgentMoveEvent(tick=1, agent_id="agent_0", x=3, y=4, is_waiting=False, remaining_dist=5)
    d = move.to_dict()
...
def test_null_telemetry_hook() -> None:
    ...
    hook.on_negotiation_end(NegotiationSessionEvent("s1", 1, "a0", "a1", 0, 0, 1, "AGREED", 1))
...
def test_async_logger_streaming(tmp_path: Path) -> None:
    ...
    with AsyncExperimentLogger(output_dir=log_dir, run_id="test_run_001") as logger:
        hook = logger.hook
        hook.on_manifest(ManifestEvent("test_commit", 42, 16, 16, 2, "SETTING_4", "SC", 5, 0, 1.0))
        hook.on_keyframe(KeyframeEvent(0, {"a0": {"pos": [0, 0]}}))
        hook.on_move(AgentMoveEvent(tick=0, agent_id="a0", x=0, y=0, is_waiting=False, remaining_dist=2))
        hook.on_move(AgentMoveEvent(tick=1, agent_id="a0", x=1, y=0, is_waiting=False, remaining_dist=1))
        hook.on_bid(OfferBidEvent("s1", 1, 1, "a0", 2, 0.0, 0.5))
        hook.on_negotiation_end(NegotiationSessionEvent("s1", 1, "a0", "a1", 1, 0, 1, "AGREED", 2))
...
def test_recording_levels_behavior() -> None:
    """Verify L09: metrics-only mode does not retain full frame history, while solution behavior is identical."""
    ...
    w_full.add_agent(HeatMapAgent("A", starts["A"], goals["A"], cfg_full.initial_tokens, cfg_full.fov_size))
    w_full.add_agent(HeatMapAgent("B", starts["B"], goals["B"], cfg_full.initial_tokens, cfg_full.fov_size))
    res_full = w_full.run()
    ...
    w_metrics.add_agent(HeatMapAgent("A", starts["A"], goals["A"], cfg_metrics.initial_tokens, cfg_metrics.fov_size))
    w_metrics.add_agent(HeatMapAgent("B", starts["B"], goals["B"], cfg_metrics.initial_tokens, cfg_metrics.fov_size))
