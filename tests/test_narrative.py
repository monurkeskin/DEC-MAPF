...
def test_absent_transfer_and_next_frame_remain_unavailable():
    recorded = payload()
    recorded['result']['telemetry_events'] = [
        event for event in recorded['result']['telemetry_events']
        if event['event_type'] != 'TOKEN_TRANSFER'
    ]
    recorded['frames'] = recorded['frames'][:1]
    story = negotiation_story(recorded)
    assert story['amount'] is None
    settlement = next(step for step in story['steps'] if step['kind'] == 'settlement')
    assert 'unavailable' in settlement['text']
    assert settlement['event_sequence'] == 4
    commitment = next(step for step in story['steps'] if step['kind'] == 'commitment')
    assert commitment['reference'] == 'unavailable'
    assert commitment['evidence'] == []
    assert commitment['tick'] == 0


def test_offline_narrative_embeds_exact_evidence_as_inert_json():
    recorded = payload(2)
    recorded['metadata']['run_id'] = '</script><script>alert("changed")</script>&'
    page = narrated_html(recorded)
    match = re.search(r'<script type="application/json" id="story-data">(.*?)</script>', page, re.DOTALL)
    assert match is not None
    assert json.loads(match[1]) == negotiation_story(recorded)
    assert recorded['metadata']['run_id'] not in page
    assert '<canvas id="grid">' in page
    assert "aria-label='Recorded negotiation walkthrough'" in page
