...
def test_edge_swap_conflict_avoidance():
    ...
    path_b = planner.search(start_b, goal_b, start_time=0, reservation_table=res_table, allow_wait=True)
    assert path_b is not None
    # Ensure Agent B waited or detoured rather than swapping head-on
    assert not (path_b.points[0] == Point(x=1, y=0) and path_b.points[1] == Point(x=0, y=0))
...
def test_hypothesis_path_invariant(gx: int, gy: int):
    ...
    path = planner.search(start, goal)
    assert path is not None
    assert path.length == start.manhattan_distance(goal)
    assert len(path.points) == (start.manhattan_distance(goal) + 1)
    # Check consecutive points are valid 1-step moves
    for i in range(len(path.points) - 1):
        d = path.points[i].manhattan_distance(path.points[i + 1])
        assert d in (0, 1)
...
def test_space_time_permanent_reservation():
    ...
    res_table.reserve_path(agent_id="Agent_Parked", path=parked_path, start_time=1, permanent=True)

    # Agent B starts at t=0 at (0, 2) and reaches (2, 2) at t=2 >= 1
    path_b = planner.search(Point(x=0, y=2), Point(x=2, y=2), start_time=0, reservation_table=res_table)
...
def test_precomputed_static_distance_table():
    """Verify O03: Precomputed static distance table provides valid distances and guides search around obstacles."""
    ...
    # Euclidean/Manhattan to (4,0) is 4, but obstacle path is at least:
    # (0,0)->(1,0)->(1,1)->(1,2)->(1,3)->(1,4)->(2,4)->(3,4)->(3,3)->(3,2)->(3,1)->(3,0)->(4,0) -> 12 steps
    dist_start = dist_table[(0, 0)]
    assert dist_start >= 10, f"True topological distance around wall must be >= 10, got {dist_start}"
