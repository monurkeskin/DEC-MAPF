...
def test_heatmap_agent_fov_weights():
    ...
    config = SimulationConfig(grid_width=10, grid_height=10, obstacles=obstacles, fov_size=5)
    env = MockEnvironment(config)
...
def test_bilateral_negotiation_success():
    ...
    conflicts = detect_conflicts({"A1": agent_1.planned_path, "A2": agent_2.planned_path})
    ...
    # Verify agreed paths now have ZERO conflicts
    agreed_conflicts = detect_conflicts({"A1": agent_1.planned_path, "A2": agent_2.planned_path})
