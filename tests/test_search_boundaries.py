...
def test_repeated_spatial_cell_keeps_time_dependent_occupancy():
    reservations = ReservationTable()
    for tick in (1, 2, 3):
        reservations.reserve_vertex("peer", Point(1, 0), tick)
    planner = SpaceTimeAStar(3, 1)
    result = planner.search(Point(0, 0), Point(2, 0), reservation_table=reservations)
    assert result is not None
    assert result.points == (Point(0, 0),) * 4 + (Point(1, 0), Point(2, 0))


def test_search_local_topology_does_not_survive_obstacle_changes():
    planner = SpaceTimeAStar(3, 2, {Point(1, 0)})
    detour = planner.search(Point(0, 0), Point(2, 0))
    assert detour is not None and detour.length == 4
    planner.obstacles.clear()
    direct = planner.search(Point(0, 0), Point(2, 0))
    assert direct is not None
    assert direct.points == (Point(0, 0), Point(1, 0), Point(2, 0))


def test_conflict_action_horizon_includes_final_state_but_no_unobserved_edge():
    paths = {
        "a": Path([Point(0, 0), Point(1, 0), Point(2, 0)]),
