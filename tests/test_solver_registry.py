...
def test_register_custom_solver(monkeypatch) -> None:
    # Registration is process-wide; a test-only plugin must not reach later tests.
    monkeypatch.setattr(registry, "_REGISTRY", registry._REGISTRY.copy())

    @register_solver(
        "DummySolver", is_centralized=True, description="Test custom solver"
    )
    class DummySolver(MAPFSolverProtocol):
        ...
        def solve(
            self, instance: MAPFInstance, config: SimulationConfig
        ) -> MAPFSolution:
            return MAPFSolution(
