"""Independent occupancy witnesses found in the pre-tournament mechanism audit."""
...
def test_incomplete_plan_does_not_imply_disappearance_during_conflict_detection(monkeypatch):
    ...
    world.add_agent(a)
    world.add_agent(b)
    a.apply_planned_path(Path([Point(0, 0)]))  # Bounded recovery left an unfinished plan.
...
def test_search_rejects_an_already_permanently_occupied_start():
    ...
    assert planner.search(Point(0, 0), Point(2, 0), start_time=7,
                          reservation_table=table) is None
    assert planner.last_search_status == "start_reserved"
    assert planner.search(Point(0, 0), Point(2, 0), start_time=7,
                          reservation_table=table, ignore_agent_id="parked") is not None
