...
def test_centralized_cbs_solver(crossing_instance: MAPFInstance):
    config = SimulationConfig(
        grid_width=6, grid_height=6, setting=SimulationSetting.SETTING_4
    )
...
def test_decentralized_negotiation_solver_unified_interface(
    crossing_instance: MAPFInstance,
):
    config = SimulationConfig(
        grid_width=6, grid_height=6, setting=SimulationSetting.SETTING_4
    )
...
def test_movingai_map_and_stern_generator():
    ...
    obstacles = generate_benchmark_map(
        width=16, height=16, obstacle_density=0.10, seed=42
    )
    ...
    pairs = generate_stern_scenario(
        16, 16, obstacles, num_agents=20, min_dist=4, max_dist=24, seed=42
    )
    assert len(pairs) == 20
    for s, g in pairs:
...
def test_external_binary_solver_parser_fixture(
    tmp_path, crossing_instance: MAPFInstance
):
    """External oracle adapter passes golden parser fixture and path validation without requiring C++ binary."""
...
""")
    ...
    solver_mock = ExternalBinarySolver(
        executable_path=mock_bin,
        solver_name="ParserFixture",
