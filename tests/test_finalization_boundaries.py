...
from tests.worker_fixtures import cpu_heavy_worker, uncooperative_worker
...
def test_legacy_import_preserves_source_nulls_and_refuses_inferred_qualification(
    tmp_path, extension
):
    frame = pd.DataFrame(
        {"seed": [42, 51], "success": [True, False], "cost": [7, None]}
    )
    source = tmp_path / f"historical.{extension}"
    getattr(frame, f"to_{'excel' if extension == 'xlsx' else extension}")(
        source, index=False
    )
    ...
    assert (
        quarantine_table(source, tmp_path / "archive", "historical-empirical")
        == receipt
    )
...
def test_article_selection_freezes_rejected_rows_nested_prefixes_and_source_hashes(
    tmp_path,
):
    ...
    assert source["selected_prefixes"]["2"] == [
        {"row": 2, "distance": 7},
        {"row": 3, "distance": 7},
    ]
...
def test_concurrent_admission_same_idempotency_key_has_one_attempt(tmp_path):
    ...
    supervisor = JobSupervisor(
        repo, max_workers=1, max_pending=2, worker=uncooperative_worker
    )
    try:
        request = JobSubmissionRequest(
            scenario_id="crossing-2a", solver_id="CBS", timeout_sec=20
        )
        with ThreadPoolExecutor(max_workers=4) as pool:
            results = list(
                pool.map(lambda _: supervisor.submit(request, "same-key"), range(8))
            )
...
def test_stopping_shared_experiment_preserves_other_jobs_and_retry_parent(tmp_path):
    ...
    try:
        ...
        assert (
            retry["effective_config"]
            == repo.get_job(first["job_id"])["effective_config"]
        )
    finally:
        driver.close()
        supervisor.close()
...
def test_current_telemetry_union_rejects_unknown_major_and_missing_or_wrong_field():
    ...
    for changed in [
        dict(record, schema_version="telemetry-99"),
        dict(record, x="0"),
...
def test_http_remains_responsive_with_two_busy_children(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        deadline = time.monotonic() + 10
        while any(
            not (repo.staging / f"{j['attempt_id']}.busy").exists() for j in jobs
        ):
            assert time.monotonic() < deadline
            time.sleep(0.02)
        assert all(repo.get_job(j["job_id"])["state"] == "running" for j in jobs)
        latencies = []
        for _ in range(10):
        ...
        (tmp_path / "responsiveness.json").write_text(
            __import__("json").dumps({"seconds": latencies, "worker_count": 2})
        )


def test_quota_scan_tolerates_file_unlinked_between_listing_and_stat(
