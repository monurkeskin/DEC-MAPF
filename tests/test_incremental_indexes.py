...
def legacy_observation(world, aid):
    # Force the preserved public full-scan implementation for independent comparison.
    from mapf.core.observations import Message, Observation

    agent = world.agents[aid]
    radius = world.config.fov_size // 2
    messages, obstacles = [], set()
    for point in world.config.obstacles:
        if (
            abs(point.x - agent.current_pos.x) <= radius
            and abs(point.y - agent.current_pos.y) <= radius
        ):
            obstacles.add(point)
    for other_id, other in world.agents.items():
        if (
            abs(other.current_pos.x - agent.current_pos.x) > radius
            or abs(other.current_pos.y - agent.current_pos.y) > radius
        ):
            continue
        reached = other.get_state().reached_goal
        if reached and not world.config.setting.disappear_at_target:
            obstacles.add(other.target_pos)
        if other_id == aid or (reached and world.config.setting.disappear_at_target):
            continue
