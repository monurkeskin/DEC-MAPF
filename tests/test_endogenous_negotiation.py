...
def test_endogenous_impasse_detection():
    ...
    config = SimulationConfig(
        grid_width=10, grid_height=10, setting=SimulationSetting.SETTING_4
    )
    env = MockEnv(config)
    ...
    # Repeated stationary offers should trigger the legacy impasse rule by round 4.
    total_bids = len(agent_a.bids_made) + len(agent_b.bids_made)
    assert total_bids <= 4, (
        f"Expected impasse within 4 rounds, but took {total_bids} rounds"
    )


def test_multiround_bargaining_without_premature_truncation():
    """Legacy bargaining can reach agreement after more than five offers."""
    config = SimulationConfig(
        grid_width=10, grid_height=10, setting=SimulationSetting.SETTING_4
    )
    env = MockEnv(config)
