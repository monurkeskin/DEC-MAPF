"""Comprehensive unit and integration tests for Phase C: GUI Correctness Gate (G01 - G10).

Covers:
- G01: Unified Pydantic response models & contract records.
- G02: t=0 initial state contract, t -> t+1 movement, and frame alignment.
- G03: Four-state classification (solved, failed, truncated, invalid) and actual goal coordinate audit.
- G04: Agent lifecycle replay states under DaT (disappeared) vs noDaT (parked).
- G05: Distinct visual layer data: local heat grid, executed and planned paths.
- G06: Explicit comparative branch parameters without hidden commitment overrides.
- G07: Provenance run_id and canonical instance_hash in run header.
- G08: CPU solve execution via background worker without blocking FastAPI loop.
- G09 & G10: Append-only SSE event buffer, Last-Event-ID cursor, multi-client parity, and error propagation.
"""
...
def test_t0_frame_contract_and_makespan_alignment():
    """G02: Ensure frame 0 represents t=0 initial state, and frame count == makespan + 1."""
...
def test_agent_lifecycle_dat_vs_nodat():
    """G04: Verify disappearance under DaT (Setting 4) vs permanent parked obstacle under noDaT (Setting 2)."""
...
def test_four_state_solution_classification():
    """G03: Verify solved, invalid, truncated, and failed classifications."""
    ...
    res_inv = build_solver_run_result("T", "T", instance, sol_invalid, SimulationSetting.SETTING_4, 5)
    ...
    res_trunc = build_solver_run_result("T", "T", instance, sol_trunc, SimulationSetting.SETTING_4, 5, max_steps=50)
    ...
    sol_solved = MAPFSolution(
        ...
        success=True,
        paths={
            "Agent_01": Path(points=[Point(0, 0), Point(0, 1), Point(1, 1), Point(1, 0)]),
            "Agent_02": Path(points=[Point(1, 0), Point(1, 0), Point(0, 0), Point(0, 0)]),
        },
        makespan=3,
    )
    res_solved = build_solver_run_result("T", "T", instance, sol_solved, SimulationSetting.SETTING_4, 5)
...
def test_gui_simulate_endpoint_and_comparison(test_client: TestClient):
    """G01, G06, G07: Validate API request/response schema, provenance headers, and comparison without overrides."""
    payload = {
        "grid_width": 8,
        "grid_height": 8,
        "agent_count": 2,
    ...
    # Verify Pydantic schema contract (G01)
...
async def test_sse_event_buffer_last_event_id_and_multi_client():
    """G09 & G10: Test append-only event buffer, Last-Event-ID catchup, and multi-client parity."""
    mgr = BenchmarkTaskManager()
    task = mgr.create_and_start_task("quick_test")
    assert task.task_id.startswith("task-")

    # Give worker thread a moment to generate initial events
    await asyncio.sleep(0.5)

    # 1. Subscriber A connects from beginning (last_event_id = None)
    events_client_a: list[str] = []
    async for chunk in mgr.stream_task_events(task.task_id, last_event_id=None):
        events_client_a.append(chunk)
        if "suite_finished" in chunk or "error" in chunk:
            break
    ...
    # 2. Subscriber B connects with Last-Event-ID = 2 (should only get events starting at id 3)
    events_client_b: list[str] = []
    async for chunk in mgr.stream_task_events(task.task_id, last_event_id=2):
        events_client_b.append(chunk)
        if "suite_finished" in chunk or "error" in chunk:
            break
