...
def completed(tmp_path):
    ...
    manifest = compile_experiment({"name": "progress witness", "scenarios": [{
        "scenario_id": "crossing-2a", "solver_id": "CBS", "timeout_sec": 10}],
        "budget": {"workers": 1, "wall_seconds": 20, "max_trials": 1, "disk_mb": 64}})
    assert service.execute(manifest)["successful"] == 1
    return service, manifest["experiment_id"]


def test_repeated_progress_does_not_reread_unchanged_verified_artifact(completed, monkeypatch):
...
def test_compact_latest_attempt_does_not_drop_attempt_lineage(completed):
    ...
    retry = {**first, "job_id": "job-" + "f" * 32, "state": "failed", "attempt_index": 1,
             "created_at": first["created_at"] + 1, "error": "controlled infrastructure failure",
             "parent_job_id": first["job_id"]}
    with service.repository.connect() as db:
        db.execute("INSERT INTO jobs VALUES(?,?,?,?,?)", (retry["job_id"], None, "test", encode(retry).decode(), retry["created_at"]))
