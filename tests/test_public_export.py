...
_spec = importlib.util.spec_from_file_location(
    "public_export", Path(__file__).parents[1] / "scripts/public_export.py"
)
...
def repo(tmp_path):
    ...
    for name, data in {
        "pyproject.toml": 'name = "example"',
        "src/mapf/cli.py": "# public entry",
        "benchmarks/results/raw.csv": "private observation",
        "README.md": "# Example",
    }.items():
        p = source / name
...
def test_export_reads_committed_files_without_raw_data_history_or_dirty_changes(
    tmp_path,
):
...
@pytest.mark.parametrize(
    "payload",
    [
        b"co-" + b"author preview",
        b"/Users/" + b"somebody/notes",
...
def test_initial_publication_rejects_internal_chronology_and_private_commit_literals(
    payload,
):
...
def test_current_alpha_and_upstream_identity_remain_valid_documentation():
    payload = (
        b"Alpha 0.1.0a2; /api/v1; taop-v2; Python 3.12; "
