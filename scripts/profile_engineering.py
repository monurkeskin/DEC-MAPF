    ...
    for spec in specifications():
        for level in ("metrics-only", "full-trace"):
            ...
            for repetition in range(-1, args.repetitions):
                ...
                value["measured_metrics"]["solver_diagnostics"].pop(
                    "heat_recording", None
                )
                signature = digest(value)
                expected = signatures.setdefault(spec["name"], signature)
                if signature != expected:
                    raise AssertionError(
                        f"Recording/repetition changed behavior: {spec['name']}"
                    )
                row = {
                    "fixture": spec["name"],
                    "recording": level,
            ...
            tracemalloc.stop()
            rows[-1]["separate_instrumented_peak_bytes"] = peak
    stream = io.StringIO()
        ...
        json.dumps(
            {
                ...
                "timing_scope": "Warmup excluded; timing runs uninstrumented; separate cProfile/tracemalloc. Existing native workload remains running on this host.",
