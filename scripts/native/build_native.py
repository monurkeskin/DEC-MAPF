...
def main():
    ...
    for name, (repo, revision) in SOURCES.items():
        ...
        settings = (
            ...
            if name == "eecbs-stay" or args.stage == "corrected"
            else [2]
        )
        for setting in settings:
            build = ROOT / "build" / f"{args.stage}-{name}-s{setting}"
            definitions = [
            ...
            log = ROOT / f"{args.stage}-{name}-s{setting}-build.log"
