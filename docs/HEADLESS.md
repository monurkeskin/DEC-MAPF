...
The CLI is a complete research workflow. It freezes scenario/configuration combinations, supervises isolated solver processes, independently validates trajectories, preserves attempts and exports every planned outcome. You do not need FastAPI, a browser, Node, or a running GUI server.
...
Planning does not run a solver. Read `trials[*].plan.scenario`, `effective_config`, `warnings`, `budget`, `sampling` and `excluded` in the manifest. Every scenario is an ordered coordinate snapshot. The manifest binds these inputs to source bytes and configuration/version receipts.
...
The command waits for completion or a study budget/interrupt condition. A terminal summary includes `planned`, `counts`, `successful`, `success_rate` and elapsed driver time. `completed` means the job returned an artifact; inspect `success` and `validation_status` for the scientific result. Failed or timed-out trials remain part of the planned denominator.
...
The CSV includes unsuccessful and not-admitted trials, execution/validation states, attempt counts, run identities and effective parameters. Nested diagnostics are JSON values inside CSV fields. Do not read only completed run files and call their count the experiment's denominator.
...
Repeated progress queries project only current attempt fields and cache a verified success scalar while the artifact's file identity and metadata remain unchanged. Full histories and artifacts remain available for export; missing artifacts remain in the planned denominator. This reduces monitoring I/O, not the solver's search work or declared scientific timeout.
...
Next: declare your real sampling population before expanding the matrix.
