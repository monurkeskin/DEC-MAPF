...
| `export_bundle`, `check_run_bundle`, `check_solver` | Portable evidence and finite conformance witnesses |

Register top-level factories in an importable module; multiprocessing spawn must execute that registration in each worker. Protect execution with `if __name__ == "__main__"`. Never define a new algorithm merely to test the extension boundary. The [registered CBS example](../examples/05_registered_solver_study.py) wraps an existing solver unchanged and demonstrates worker composition, validation and export.
