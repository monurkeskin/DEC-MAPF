...
- Documentation gallery captures show selected validated modern runs from article settings with [image provenance](../assets/gallery/provenance.json). They are not a substitute for user acceptance of every workflow or a scientific comparison.
...
- Current events include actual messages, policy decision components, signed transfers and local observations. Full Java strategy equivalence remains unclaimed; see [protocol conformance](../TAOP-CONFORMANCE.md).
...
Read the [workspace contracts](CONTRACTS.md) and [metric definitions](../METRICS.md) when interpreting an exported result.
