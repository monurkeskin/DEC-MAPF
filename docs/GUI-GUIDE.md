...
An opened historical run can show **Execution: idle** and **Not connected** while its saved solver/validation evidence is fully loaded. Those labels refer to the current live job stream, not to loss of the saved replay. Use the result's validation receipt and run ID to interpret it.
...
The header distinguishes a **Saved replay** from a **Live job**. Playing a saved replay never launches a solver. An idle process indicator is compatible with a saved valid result. See the [glossary and legend](GLOSSARY.md) and [recorded negotiation walkthrough](NEGOTIATION-WALKTHROUGH.md).
