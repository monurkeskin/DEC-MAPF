...
Before a release, inspect the exact commit's checks, licenses, citations, documentation and package contents, then tag that reviewed commit. These workflows test and build packages; publishing a release or distributing a package is a separate maintainer action.

When simplifying tests, review the retained behavioral assertions as well as the
coverage report. Compare line and branch execution, inspect slow cases in Codecov
Test Analytics, and check CodeScene results for the same source revision. Targeted
fault checks should run in a disposable copy; never mutate a running experiment.
The [contributor guide](../CONTRIBUTING.md#what-to-verify) explains how to preserve
useful cases without treating the test count or coverage score as the objective.
