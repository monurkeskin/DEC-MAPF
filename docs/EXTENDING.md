...
minimal `02_custom_negotiation_agent.py` initialization sketch as a qualified
...
## Custom space-time reservations

`SpaceTimeAStar` honors `ReservationTable` subclasses and instance overrides of
`is_vertex_reserved` and `is_edge_conflict`. These extensions bypass the primitive
occupancy shortcut and candidate cache, so changing a reservation query cannot
reuse an incompatible path. The unmodified table retains the fast path.

When using `permanent_at_goal=True`, a custom table must also implement
`can_park(location, tick, ignore_agent_id) -> bool`. Return true only if the cell
can remain occupied from that tick onward under your reservation policy. Checking
one arrival tick cannot establish indefinite availability; a missing predicate
raises `TypeError`. Include vertex, reverse-edge and future goal-occupancy
counterexamples when qualifying a custom table.
