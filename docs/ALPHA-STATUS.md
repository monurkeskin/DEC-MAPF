...
| `dec-mapf`, version `0.1.0a1` | Python distribution and current alpha version |
...
The GUI runs locally. A hosted multi-user service, a software DOI and publication of raw experimental datasets are separate capabilities, not prerequisites for using the framework. See [installation](INSTALLATION.md) for source and wheel workflows, and [reporting an issue](MAINTENANCE.md) for support.
