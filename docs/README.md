...
Use DEC-MAPF to run decentralized and centralized methods on shared scenarios,
validate their trajectories and compare results. The Python API and CLI support
the full experiment workflow; the optional GUI reads the same saved runs.
Start with the workflow below that matches your task. [Alpha status](ALPHA-STATUS.md)
describes compatibility and scientific scope.
...
| I want to run an experiment | [Installation](INSTALLATION.md) → [Headless tutorial](HEADLESS.md) → [Reproducibility](../REPRODUCIBILITY.md) | A saved study specification, one row for every planned trial and matched comparisons |
...
| I have my own maps and agents | [Scenarios](SCENARIOS.md) → [Parameters](PARAMETERS.md) → [Solvers](SOLVERS.md) | Validated inputs with recorded hashes and supported settings |
...
*32×32 cells, 205 blocked cells (~20%), 80 agents, HeatMap with ZC and FoV 5. A selected valid modern solution from the article-verification population. [Selection and provenance](GALLERY.md#selection-and-recording) describe how this example was selected and checked.*
...
For a first code contribution, follow the [code reading order and design conventions](CODE-GUIDE.md), then the [contribution checks](../CONTRIBUTING.md).
