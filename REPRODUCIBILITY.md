...
The `article-spec` command constructs **fresh filtered main-map prefixes** from explicit MovingAI scenario files. It selects eligible rows, records exclusions and freezes generated rosters. Such a selection can define a new article-configured study, but is not exact import of historical complete rosters. Do not label it `article-rosters`, reuse a main-map filter for the appendix, or infer the paper's entire scenario population from one Java branch.
