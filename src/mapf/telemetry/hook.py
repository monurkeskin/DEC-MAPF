"""Optional event recording through null or queued hooks.

Null callbacks perform no event I/O. Queued callbacks hand events to the
asynchronous writer; event construction and callback calls still have a cost.
"""
...
class TelemetryHook(Protocol):
    ...
    def on_keyframe(self, event: KeyframeEvent) -> None:
        """Record a replay keyframe containing the available agent state."""
        ...
...
class NullTelemetryHook:
    """Disabled recording adapter whose callbacks perform no event I/O."""
