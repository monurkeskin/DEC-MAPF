...
def register_solver(
    ...
    *,
    ...
    """Decorator to register a solver factory function or class in the MAPF ecosystem.

    Example:
        @register_solver("MyNovelSolver", is_centralized=False, description="Novel decentralized MAPF algorithm")
        class MyNovelSolver(MAPFSolverProtocol):
            ...
    """
    ...
    def decorator(
        ...
        if not name.strip() or name != name.strip() or name.lower() in _REGISTRY:
            raise ValueError(f"Solver name is empty, padded or already registered: {name!r}")
...
def solver_metadata(name: str) -> SolverMetadata:
    ...
    if key not in _REGISTRY:
        ...
        raise KeyError(f"Solver '{name}' not found in registry. Available solvers: {available}")
...
def create_from_inputs(name: str, inputs: dict[str, Any]) -> MAPFSolverProtocol:
    ...
    return meta.factory(**{target: inputs[source] for source, target in meta.constructor_inputs})
...
def get_solver(name: str, **kwargs: Any) -> MAPFSolverProtocol:
    ...
    Args:
        name: Name of the registered solver (e.g. 'HeatMap', 'CBS', 'EECBS').
...
def _register_builtins() -> None:
    ...
    register_solver(
        name="Greedy",
        is_centralized=False,
        description="Decentralized negotiation with greedy non-concession strategy",
    )(lambda **kw: DecentralizedNegotiationSolver(strategy="Greedy", **kw))

    register_solver(
        name="Conceder",
        is_centralized=False,
        description="Decentralized negotiation with rapid concession strategy",
    ...
    register_solver(
        name="CBS",
        is_centralized=True,
        description="Optimal Conflict-Based Search with Space-Time A* low-level search (Sharon et al. 2015)",
    ...
    register_solver(
        ...
        constructor_inputs=(("timeout_sec", "time_limit_sec"), ("suboptimality", "suboptimality")),
