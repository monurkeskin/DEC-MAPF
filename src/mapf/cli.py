"""CLI for solver execution, saved studies, diagnostics and the local GUI."""
...
def list_solvers_command(args: argparse.Namespace) -> int:
    """List the solvers registered in this process."""
...
def diagnostics_command(args: argparse.Namespace) -> int:
    """Summarize saved events and write a standalone diagnostics report."""
    from pathlib import Path
...
def verify_command(args: argparse.Namespace) -> int:
    """Run Ruff, mypy and pytest from the source checkout."""
    import subprocess
...
def main() -> None:
    ...
    # Subcommand: solvers
    p_solvers = subparsers.add_parser("solvers", help="List registered solvers")
    p_solvers.set_defaults(func=list_solvers_command)

    # Subcommand: diagnostics
    p_diag = subparsers.add_parser(
        "diagnostics", help="Summarize recorded event telemetry"
    )
    p_diag.add_argument(
    ...
    p_dash.add_argument(
        ...
        type=int,
        choices=range(1, 7),
        help="Owned solver processes (default 2)",
    )
    p_dash.add_argument(
        "--resource-policy",
        help="JSON resource-admission policy; requires resources extra",
    )
