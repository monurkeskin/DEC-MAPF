...
def parameter_descriptor(name: str) -> ParameterDescriptor:
    ...
    return _parameter_descriptor(name, JobSubmissionRequest.model_json_schema()["properties"])


def _parameter_descriptor(name: str, properties: dict[str, Any]) -> ParameterDescriptor:
    raw = properties[name]
    ...
    return ParameterDescriptor(name, label, kind, deepcopy(raw.get("default")), description,
        options=deepcopy(schema.get("enum")), min_value=schema.get("minimum"),
...
def _registered_capability(name: str, properties: dict[str, Any]) -> SolverCapability:
    metadata = solver_metadata(name)
    central = metadata.is_centralized
    ...
    return SolverCapability(
        name if central else f"Decentralized-{name}",
        metadata.display_name or name,
        "centralized" if central else "decentralized",
        central, not central, not central, not central, True,
        metadata.supports_suboptimality, metadata.description,
        [_parameter_descriptor(key, properties) for key in names],
    )


def _native_capability(profile: NativeSolverProfile) -> SolverCapability:
    return SolverCapability(profile.solver_id, profile.display_name,
...
class SolverRegistry:
    ...
    def list_capabilities() -> list[SolverCapability]:
        # Reuse this query's schema, never stale metadata from a previous query.
        properties = JobSubmissionRequest.model_json_schema()["properties"]
        return [
            _registered_capability(name, properties) for name in list_solvers()
        ] + [_native_capability(profile) for profile in native_profiles()]
    ...
    def get_capability(solver_id: str) -> SolverCapability | None:
        ...
        if profile is not None:
            return _native_capability(profile)
        return _registered_capability(name, JobSubmissionRequest.model_json_schema()["properties"])
