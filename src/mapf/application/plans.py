...
def preview(
    ...
    _check_timeout(request, cap)
    snap = _requested_snapshot(request, repository)
    config = _effective_config(request, snap, cap)
    inactive = _inactive_parameters(cap)
    definition = {
    ...
    return {
        ...
        "budget": _compute_budget(request, cap, len(snap["starts"])),
    }


def _check_timeout(request: JobSubmissionRequest, cap: SolverCapability) -> None:
...
def _requested_snapshot(request: JobSubmissionRequest, repository: RunRepository | None) -> dict[str, Any]:
    if not request.scenario_id:
        return scenario_snapshot(request)
    snap = _saved_snapshot(request.scenario_id, repository)
    ...
    if errors:
        raise ValueError("; ".join(errors))
    return ScenarioService.snapshot_instance(inst, setting, name=snap["name"])


def _saved_snapshot(scenario_id: str, repository: RunRepository | None) -> dict[str, Any]:
    snap = builtins().get(scenario_id)
    if snap is None and repository is not None:
        snap = repository.get_scenario(scenario_id)
    if snap is None:
        raise KeyError(scenario_id)
    return snap


def _effective_config(request: JobSubmissionRequest, snap: dict[str, Any], cap: SolverCapability) -> dict[str, Any]:
...
def _inactive_parameters(cap: SolverCapability) -> list[str]:
    inactive = (
...
def _compute_budget(request: JobSubmissionRequest, cap: SolverCapability, agents: int) -> dict[str, Any]:
    return {
