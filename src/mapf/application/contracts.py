"""Pydantic schemas and event models for MAPF Researcher Studio GUI.

Defines unified types across REST endpoints, SSE streams, frame scrubbing,
and independent solution verification receipts.
"""
...
class FrameSnapshot(BaseModel):
    """Immutable state snapshot at discrete simulation tick t (starting at t=0)."""
...
class JobSubmissionRequest(ScenarioValidateRequest):
    ...
    timeout_sec: float | None = Field(default=5.0, ge=0.1, le=600.0,
        description="Optional whole-process safety budget; null removes this cap for decentralized jobs only")
    negotiation_deadline_sec: float = Field(default=60.0, ge=0.01, le=600.0,
