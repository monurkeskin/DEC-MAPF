...
class SpatialComponent:
    """An agent's current position, goal and movement priority."""
...
class PathPlanningComponent:
    """Planned trajectory, initial length and compatibility reservation state."""
...
class NegotiationComponent:
    """Token balance, strategy name and negotiation counters."""
...
class SimFrame:
    """Recorded state for one simulation tick.

    Fields cannot be reassigned, but nested dictionaries and lists remain mutable.
    Treat nested payloads as read-only after capture; copy them before mutation.
    """
