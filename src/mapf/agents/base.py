...
class BaseAgent(ABC, AgentProtocol):
    """Own agent state, movement, replanning and commitment handling."""
    ...
    def on_contract_agreed(self, contract: Contract, current_time: int) -> None:
        ...
        if self._agent_id not in (contract.agent_a, contract.agent_b):
            return
        is_a = contract.agent_a == self._agent_id
        delta = -contract.token_transfer if is_a else contract.token_transfer
        if self._current_tokens + delta < 0:
            raise ValueError("Token overdraft")
        self._planned_path = contract.path_a if is_a else contract.path_b
        self.adjust_tokens(delta)
        if contract.accepted_by is not None and contract.accepted_by != self._agent_id:
            return
        other_path = contract.path_b if is_a else contract.path_a
        points = (contract.allocated_path or other_path).points
        if self._commitment_type == CommitmentType.DYNAMIC:
            points = self._dynamic_allocation(points, contract, current_time)
        opponent = contract.agent_b if is_a else contract.agent_a
    ...
    def _dynamic_allocation(
        self, points: tuple[Point, ...], contract: Contract, current_time: int
    ) -> tuple[Point, ...]:
        horizon = (
            contract.conflict_tick - current_time
            if contract.conflict_tick is not None
            else self._last_conflict_step
        )
        return points[: max(0, horizon) + 1]

    def _calculate_concession_rate(self, token_cost: int) -> float:
        """Shared PathAware/HeatMap ratio of remaining tokens to remaining path."""
        token_rate = max(0.0, float(self._current_tokens - token_cost)) / float(
            max(1, self._initial_tokens)
        )
        path_rate = max(0.01, float(self._planned_path.length)) / float(
            max(1, self._initial_path_length)
        )
        return token_rate / path_rate

    def _accept_response(
        self, candidate: Path, bid: Bid, default_protocol: str
    ) -> bool:
        """Apply the shared response policy after a strategy records a feasible candidate."""
        reference = getattr(self, "_negotiation_reference_path", self._planned_path)
        delay = candidate.length - reference.length
        protocol = getattr(self, "_active_negotiation_protocol", default_protocol)
        if protocol in {"taop-v1", "taop-v2"}:
            accepted = self._taop_response(delay, protocol)
        else:
            accepted = self._legacy_response(delay, bid)
        if accepted:
            self._planned_path = candidate
        return accepted

    def _taop_response(self, delay: int, protocol: str) -> bool:
        usage = getattr(self, "_acknowledged_usage", 0)
        concede = protocol == "taop-v2" and (
            usage >= self._current_tokens
            or self._calculate_concession_rate(usage) < 1.0
        )
        if concede and delay > 0:
            self._last_decision["reason"] = "CONCEDE_FEASIBLE_RESPONSE"
            self._last_decision["concession_rate"] = self._calculate_concession_rate(
                usage
            )
        return delay <= 0 or concede

    def _legacy_response(self, delay: int, bid: Bid) -> bool:
        if delay <= 0 or bid.token_offered >= delay:
            return True
        rate = self._calculate_concession_rate(token_cost=0)
        max_delay = max(2, min(4, self._initial_path_length // 4))
        willing = rate < 1.0 or bid.round_num >= 2 or delay <= 2
        return delay <= max_delay and willing

    def _prune_commitments(self, current_tick: int) -> None:
        self._commitments = {
            key: record
            for key, record in self._commitments.items()
            if record.is_active(current_tick)
        }

    def respects_commitments(
        self, path: Path, current_tick: int, *, stay_at_goal: bool
    ) -> bool:
        return not any(
            record.conflicts_with(path, current_tick, stay_at_goal=stay_at_goal)
    ...
    def propose_response(
        self, bid: Bid, env: EnvironmentProtocol, current_time: int
    ) -> BidDecision:
        ...
        try:
            ...
            return BidDecision(
                accepted=accepted,
                proposed_path=self.planned_path,
                reason=str(
                    components.get("reason", "ACCEPT" if accepted else "REJECT")
                ),
...
def goal_reached(agent: AgentProtocol) -> bool:
    ...
    if (
        ...
        and type(agent).get_state is BaseAgent.get_state
