...
class WorldSimulation(EnvironmentProtocol):
    ...
    def __init__(self, config: SimulationConfig) -> None:
        ...
        self._negotiator = BilateralNegotiationSession(
            max_rounds=config.negotiation_round_limit,
            protocol=config.negotiation_protocol,
    ...
    def detect_conflicts(
        ...
        *,
        ...
        return self._conflict_index.detect(
            paths=paths,
            goals=goals,
    ...
    def stop_negotiation(self, reason: str, detail: dict[str, Any]) -> None:
        ...
        self._last_negotiation_failure = {
            ...
            "reason": reason,
            "scope": "execution_stopped; no initial-instance infeasibility proof",
            **detail,
        }
        self.telemetry_hook.on_diagnostic(
            DiagnosticEvent(
    ...
    def record_movement_repair(self, repair: dict[str, Any]) -> None:
        ...
        self._movement_diagnostics["remaining_agent_holds"] += len(
            repair["remaining_holds"]
        )
        self._movement_diagnostics["remaining_blocked_agents"] += len(
            repair["remaining_blocked_agents"]
        )
        for component in repair["components"]:
            self._movement_diagnostics[component["status"]] += 1
        self.telemetry_hook.on_diagnostic(
            DiagnosticEvent("MOVEMENT_REPAIR", self.current_time, repair)
        )

    def record_motion(self, *, stationary: bool) -> None:
        self._no_motion_streak = self._no_motion_streak + 1 if stationary else 0
        self._longest_no_motion_streak = max(
            self._longest_no_motion_streak, self._no_motion_streak
        )
    ...
    def _parked_targets(self) -> set[Point]:
        if self.config.setting.disappear_at_target:
            return set()
        return {a.target_pos for a in self._agents.values() if goal_reached(a)}

    def get_fov_obstacles(self, center: Point, fov_size: int) -> set[Point]:
        radius = fov_size // 2
        obstacles = self._config.obstacles | self._parked_targets()
        return {point for point in obstacles if self._in_view(point, center, radius)}

    @staticmethod
    def _in_view(point: Point, center: Point, radius: int) -> bool:
        return max(abs(point.x - center.x), abs(point.y - center.y)) <= radius

    def _broadcast_visible(
        self, agent: AgentProtocol, center: Point, radius: int
    ) -> bool:
        if not self._in_view(agent.current_pos, center, radius):
            return False
        return not (goal_reached(agent) and self.config.setting.disappear_at_target)
    ...
    def get_fov_broadcasts(
        ...
        for a_id, agent in self._agents.items():
            if a_id == requesting_agent_id:
                continue
            if self._broadcast_visible(agent, center, radius):
    ...
    def deliver_message(
        ...
        *,
        ...
        message = Message(
            ...
            recipient,
            self.current_time,
            tuple((p.x, p.y) for p in path.points),
            kind,
            session_id,
            acknowledgement,
        )
    ...
    def for_agent(self, agent_id: str) -> LocalEnvironment:
        ...
        standard_queries = all(
            getattr(getattr(self, name), "__func__", None)
            is getattr(WorldSimulation, name)
            for name in ("get_fov_obstacles", "get_fov_broadcasts")
        )
        if standard_queries:
            observation = self._observation_index.observe(
                self._agents, self.config, self.current_time, agent_id
            )
        ...
        messages = tuple(
            ...
            for sender, path in self.get_fov_broadcasts(
                agent.current_pos, self.config.fov_size, agent_id
            ).items()
        )
    ...
    def record_settlement(self, receipt: dict[str, Any]) -> None:
        ...
        self._telemetry_hook.on_diagnostic(
            DiagnosticEvent("TOKEN_TRANSFER", self.current_time, receipt)
        )

    def record_decision_heat(
        self, agent: AgentProtocol, tick: int, session_id: str
    ) -> None:
        record = self._heat_recorder.capture(agent, tick, session_id)
        if record is not None:
            self._telemetry_hook.on_diagnostic(
                DiagnosticEvent("DECISION_HEAT", tick, record)
            )

    def observation_snapshot(self) -> dict[str, Any]:
        return {
            aid: observation.to_dict()
            for aid, observation in self._observations.items()
        }
    ...
    def initialize(self) -> None:
        ...
        for agent in self._agents.values():
            ...
            empty = Observation(
                ...
                0,
                agent.current_pos,
                frozenset(self.config.obstacles),
                (),
            )
    ...
    def step(self) -> bool:
        ...
        active_agents = {
            a_id: a for a_id, a in self._agents.items() if not goal_reached(a)
        }

        if not active_agents:
            return True

        if self._goals_disconnected(active_agents):
            return False
    ...
    def _goals_disconnected(self, active: dict[str, AgentProtocol]) -> bool:
        # Reached-state stopping condition, never information delivered to agents.
        # Only absorbing arrivals are permanent obstacles; finite reservations
        # and disappearing agents cannot establish this condition.
        parked = frozenset(self._parked_targets())
        if not parked:
            return False
        disconnected = self._connectivity.disconnected(
            {aid: a.current_pos for aid, a in active.items()},
            {aid: a.target_pos for aid, a in active.items()},
        ...
        self._last_reachability_failure = {
            ...
            "parked_cells": [
                [p.x, p.y] for p in sorted(parked, key=lambda p: (p.x, p.y))
            ],
    ...
    def run(self) -> dict[str, Any]:
        """Run until completion, a recorded stopping condition or the step limit."""
        self.initialize()
    ...
    def _validate_trajectories(self) -> bool:
        ...
        executed_paths = {
            aid: Path(points=hist) for aid, hist in self._path_history.items()
        }
        ...
        self._collision_count = sum(
            ...
            if e.error_type in ("vertex_collision", "edge_collision")
        )

        return val_result.is_valid

    def _result(self) -> dict[str, Any]:
        valid = self._validate_trajectories()
        solved_count = sum(1 for a in self._agents.values() if goal_reached(a))
        total_count = len(self._agents)
        success = (
            (solved_count == total_count) and valid and (self._collision_count == 0)
        )

        total_path_length, initial_path_length, norm_path_diff = self._path_totals()
        nego_by_step, broadcast_ratio = self._negotiation_counts(total_count)

        measured = self._communication.metrics(
            self._path_history, {aid: a.tokens for aid, a in self._agents.items()}
        )
        return {
            "success": success,
            ...
            "successful_negotiations": sum(
                1 for n in self._negotiation_history if n["success"]
            ),
            ...
            **measured,
            ...
            "heat_recording": {
                "enabled": self._heat_recorder.enabled,
                "entry_limit": self._heat_recorder.entry_limit,
                "entries": self._heat_recorder.entries,
                "record_limit": self._heat_recorder.record_limit,
                "records": self._heat_recorder.total,
                "omitted": self._heat_recorder.omitted,
            },
            "solver_diagnostics": self._diagnostics(),
            # Stopping by arrival is distinct from independent physical validity.
            "termination_reason": self._termination_reason(solved_count, total_count),
            "initial_snapshot": self._initial_snapshot
            if self.config.recording_level == RecordingLevel.FULL_TRACE
            else {},
            "nego_by_step": nego_by_step,
            "frames": [f.to_dict() for f in self._frames]
            if self._config.recording_level != RecordingLevel.METRICS_ONLY
            else [],
        }

    def _path_totals(self) -> tuple[int, int, float]:
        total = sum(max(0, len(hist) - 1) for hist in self._path_history.values())
        initial = sum(a.get_state().initial_path_length for a in self._agents.values())
        difference = float(total - initial) / float(initial) if initial > 0 else 0.0
        return total, initial, difference

    def _negotiation_counts(self, total_count: int) -> tuple[dict[int, int], float]:
        by_step: dict[int, int] = {}
        contacts: dict[str, set[str]] = {aid: set() for aid in self._agents}
        for entry in self._negotiation_history:
            tick = entry["time"]
            by_step[tick] = by_step.get(tick, 0) + 1
            contacts[entry["agent_a"]].add(entry["agent_b"])
            contacts[entry["agent_b"]].add(entry["agent_a"])
        average = sum(len(c) for c in contacts.values()) / float(max(1, total_count))
        ratio = average / float(max(1, total_count - 1)) if total_count > 1 else 0.0
        return by_step, ratio
    ...
    def _termination_reason(self, solved: int, total: int) -> str:
        if solved == total:
            return "all_arrived"
        if self._stop_reason:
            return self._stop_reason
        return "bounded_replan_failed" if self._is_unsolvable else "step_limit"
