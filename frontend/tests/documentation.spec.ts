...
test("local documentation search leads to the actual study guide", async ({
  ...
  // Material's search subscribes to keyup, not the input event emitted by fill.
  // Type as a user does, independently of when the search worker becomes ready.
  await search.pressSequentially("study");
  ...
  for (let index = 0; index < 4; index++) {
    ...
    await expect(
      page.getByRole("heading", {
        name: /^Recorded 80-agent simulations/,
      }),
    ).toBeVisible();
